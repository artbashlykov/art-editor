...
= 0.2.61 =
* В структуре рядом с корзиной — иконка «глаз»: скрытый HTML-блок остаётся в редакторе (серым), но не попадает в «Просмотр» и на сайт.

= 0.2.60 =
* Превью (Редактирование и Просмотр): отправка формы больше не открывает редактор внутри iframe. Живой сайт не меняется.

= 0.2.59 =
* Вкладка «Просмотр»: якорные ссылки (#section) снова прокручивают страницу. Внешние ссылки по-прежнему не уводят iframe. Режим «Редактирование» не менялся.

= 0.2.58 =
* Инлайн-скрипты HTML-блоков остаются на исходном месте (currentScript / closest снова работают в квизах). Скрипты из head по-прежнему дописываются в конец блока.

= 0.2.57 =
* Подсветка выбранного элемента на вкладке «Код» снова работает: поиск фрагмента не зависит от порядка атрибутов в outerHTML.

= 0.2.56 =
* Блокировка поста как в WordPress: общий замок с Gutenberg, диалог «Войти и перехватить», Heartbeat и истечение замка при закрытии вкладки.
...
= 0.2.53 =
* Вставка текста в блоки на канвасе всегда без форматирования (plain text): стили из Telegram и других приложений больше не попадают в HTML.
...
= 0.2.49 =
* Превью больше не прыгает вверх после правок: позиция прокрутки iframe сохраняется при пересборке.
