( function() {
	...
	function getEditInspectHeadMarkup() {
		return [
			...
			'if(event&&event.clipboardData){return event.clipboardData.getData("text/plain")||"";}',
			...
			'event.preventDefault();',
			'event.stopPropagation();',
			'insertPlainText(getClipboardPlainText(event));',
			'}',
			'function handlePlainTextDrop(event){',
			'var text;',
			...
			'},true);',
			'document.addEventListener("paste",handlePlainTextPaste,true);',
			'document.addEventListener("drop",handlePlainTextDrop,true);',
